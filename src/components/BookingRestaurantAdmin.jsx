...
const BookingsAdmin = () => {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const res = await axios.get("https://victoria-fall-backend-production.up.railway.app/api/restaubookings");
...
const formatLocalDate = (dateString) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://victoria-fall-backend-production.up.railway.app/api/restaubookings/${id}`);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://victoria-fall-backend-production.up.railway.app/api/restaubookings/${editBooking._id}`, formData);
                ...
                {currentBookings.map((b, idx) => (
                  <tr key={b._id} className="text-center border-t">
                    ...
                    <td className="py-2 px-4 border">{b.email}</td>
                     <td className="py-2 px-4 border">{b.restaurant}</td>
                    <td className="py-2 px-4 border">{b.mobile}</td>
                    <td className="py-2 px-4 border">{b.time}</td>
                    <td className="py-2 px-4 border">{formatLocalDate(b.bookingDate)}</td>
